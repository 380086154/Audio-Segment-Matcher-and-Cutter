...
- ✅ 按第一个匹配位置截去前面内容（Cut Before First Match）
- ✅ 按第一个匹配位置截去后面内容（Cut After First Match）
...
- **Cut Before First Match**：从时间轴上最早的匹配起点切到文件末尾（包含匹配段）
- **Cut After First Match**：从文件开头切到最早匹配的终点（包含匹配段）
